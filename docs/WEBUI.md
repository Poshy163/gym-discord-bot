...
| **Activity** | A game/presence feed for every tracked user: avatar with a live status dot (online/idle/dnd/offline), what they're playing right now (with the game's art when Discord exposes rich-presence assets, else a clean coloured tile), their most-played games over the window with playtime bars, and a **message log** — a scrollable feed of the user's recent messages (full content, channel, and time) with a count for the window. Defaults to the last **30 days**. Needs `ENABLE_PRESENCE_TRACKING=true` + users added via `/track start`. |
