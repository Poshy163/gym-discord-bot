...
| **Activity** | A game/presence feed for every tracked user: avatar with a live status dot (online/idle/dnd/offline), what they're playing right now (with the game's art when Discord exposes rich-presence assets, else a clean coloured tile), and their most-played games over the window with playtime bars. Needs `ENABLE_PRESENCE_TRACKING=true` + users added via `/track start`. |
