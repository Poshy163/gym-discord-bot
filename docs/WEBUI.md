...
| **Activity** | A per-member feed over the window (default last **30 days**). Every member who has chatted gets a **message log** — a scrollable feed of their recent messages (full content, channel, and time) with a count. Members added via `/track start` (with `ENABLE_PRESENCE_TRACKING=true`) *additionally* get a live status dot (online/idle/dnd/offline), what they're playing right now (with the game's art when Discord exposes rich-presence assets, else a clean coloured tile), and their most-played games with playtime bars. Message logging is for **all members regardless of opt-in** and is on by default — set `ENABLE_MESSAGE_LOGGING=false` to disable it. On startup the bot **back-scans the last 30 days** of every readable channel so the feed isn't empty on a fresh deploy (`MESSAGE_LOG_BACKFILL_DAYS`, default 30; set 0 to skip — re-runs are cheap and only fill the gap since the bot was last up). |
